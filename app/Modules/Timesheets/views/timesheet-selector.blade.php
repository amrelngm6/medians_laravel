                            <div class=" w-full py-2">
                                <div class="flex w-full gap-4">
                                    ...
                                    <a style="line-height:5px; " rel="popover" data-toggle="popover"
                                        ...
                                        href="{{route('Timesheet.show', ['model_id'=>$model->id() ?? '0', 'model_type' => get_class($model)])}}"
                                ...
                                <form action="{{route('Timesheet.store')}}" class="ajax-form " id="timesheet-form"
                                    ...
                                    <button rel="popover" data-toggle="popover" data-placement="top"
                                        data-trigger="hover" data-content="Start Timesheet for this task" type="submit"
                                        id="modal_new_target_submit" class="btn btn-primary btn-sm">
                                        <i class='bx bx-time-five'></i> Start timer <span
                                            class="h-10px inline-block rounded-full w-10px bg-{{$timesheet->status->color ?? ''}}"></span>
                                    </button>
                                </form>
                                @else
                                <a rel="popover" data-toggle="popover" data-placement="top" data-trigger="hover"
                                    ...
                                    data-confirm="End Timesheet ?"
                                    data-confirm-text="Are you sure you want to end this active Timesheet for this Task ?"
                                    href="{{route('Timesheet.end', $timesheet->id)}}?_token={{csrf_token()}}"
                                    id="track-timesheet"
                                    data-reload-link="{{$modalRoute}}"
                                    data-target-modal="task-modal"                                    
