...
@section('nav-title', 'Package Subscriptions List')
                ...
                <table id="tech-companies-1"
                    ...
                    <tbody id="data-rows">
                        ...
                        <tr>
                            ...
                            <td>{{$subscription->is_paid ? 'Paid' : 'Free'}}</td>
                            <td class=" status">
                                <div>
                                    <span class="badge round-{{$subscription->status ? 'success' : 'danger'}} px-2.5 py-0.5 inline-block text-xs font-medium rounded border ">
                                        {{$subscription->status ? 'Enabled' : 'Disabled'}}</span>
                                </div>
                            </td>
                                    ...
                                    <ul class="show-on-hover absolute   z-50  py-2 mt-0 text-start list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600"
                                        ...
                                        <li>
                                            <a class="open-modal block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 "
                                                href="{{route('PackageSubscription.edit', $subscription->id)}}">
                                                <span class="align-middle">Edit</span></a>
                                        </li>
                                        <li>
                                            <a data-path="{{route('PackageSubscription.delete', $subscription->id)}}" class="delete-item block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 "
                                                href="#!">
