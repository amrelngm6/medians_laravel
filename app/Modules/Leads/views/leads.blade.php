                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header align-items-center  gap-2 gap-md-5 w-full flex">
                            ...
                            <a class="btn btn-md btn-primary me-2 " href="{{route('Lead.create')}}">
                                New Lead </a>
                        </div>
                            ...
                            <div class="table" data-pattern="priority-columns">
                                <table
                                    ...
                                    <thead class="text-start bg-slate-100 dark:bg-zink-600">
                                        <tr>
                                            <th class="text-start w-20" data-sort="lead_id">Lead id</th>
                                            <th class="text-start Lead_name" data-sort="Lead_name">Name</th>
                                            <th class="text-start Lead_name" data-sort="Lead_name">Assigned</th>
                                            <th class="text-start" data-sort="type">Type</th>
                                            <th class="text-start" data-sort="source_id">Source</th>
                                            <th class="text-start status" data-sort="status">Status</th>
                                            
                                            @foreach ($custom_fields ?? [] as $field)
                                            @if (!empty($field->show_at_table))
                                            <th class="text-start {{$field->name}}" data-sort="{{$field->name}}">{{$field->title}}</th>
                                            @endif
                                            @endforeach
                                    ...
                                    <tbody class="list">
                                        ...
                                        <tr>
                                            ...
                                            <td class=""> <a href="{{route('Lead.tabs.overview', $lead->lead_id)}}">{{$lead->name ?? ''}}</a> </td>
                                            <td class=" ">
                                                <h6><a class=" flex gap-2" href="{{route('Staff.tabs.overview', $lead->assigned_to)}}"><img class="w-25px h-25px rounded-circle" src="{{url($lead->assigned->picture ?? 'default.png')}}" /> <span class="pt-1"> {{$lead->assigned->name}}</span></a> </h6>
                                            </td>
                                            <td class=""> {{$lead->type}} </td>
                                            <td class=""> {{$lead->source->name}} </td>
                                            <td class=" status">
                                                <span
                                                    class="badge round-danger px-2.5 py-0.5 inline-block text-xs font-medium rounded border bg-{{$lead->status_model->color ?? 'danger' }} border-transparent text-orange-500">{{$lead->status_model->name ?? ''}}</span>
                                            </td>
                                            
                                            @foreach ($custom_fields ?? [] as $field)
                                            @if (!empty($field->show_at_table))
                                            <td class=" ">{{$lead->field[$field->name] ?? ''}}</td>
                                            @endif
                                            @endforeach
                                                    ...
                                                    <ul class="show-on-hover absolute   z-50  py-2 mt-0 text-start list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600"
                                                        ...
                                                        <li>
                                                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 "
                                                                href="{{route('Lead.tabs.overview', $lead->lead_id)}}">
                                                                <span class="align-middle">Overview</span></a>
                                                        </li>
                                                        <li>
                                                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 "
                                                                href="{{route('Lead.tabs.edit', $lead->lead_id)}}">
                                                                <span class="align-middle">Edit</span></a>
                                                        </li>
                                                        <li>
                                                            <a data-path="{{route('Lead.delete', $lead->lead_id)}}" class="delete-item block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 "
                                                                href="#!">
